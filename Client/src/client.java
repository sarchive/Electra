	...
	private void mainGameProcessor()
	{
		...
		try
		{
			if(socketStream != null && stream.currentOffset > 0)
			{
				...
				amountOfDataOut += stream.currentOffset;
	...
	private void method73()
	{
		do
		{
			...
			if(backDialogID == -1)
			{
				...
				{
					if(myPrivilege >= 0) // TODO: revert privilege level
					{
	...
	private void login(String s, String s1, boolean flag)
	{
		...
		try
		{
			...
			if(k == 2)
			{
				...
				{
					playerArray[i2] = null; // remove player
					aStreamArray895s[i2] = null; //reset appearance cache
				}
...
//	entityMessage(npc);
 ...
 //		   return;
	...
	private void method91(Stream stream, int i)
	{
		while(stream.bitPosition + 10 < i * 8)
		{
			...
			if(playerArray[j] == null)
			{
				...
				if(aStreamArray895s[j] != null) // check if appearance is available from appearance cache.
					playerArray[j].updatePlayer(aStreamArray895s[j]);
			}
			playerIndices[playerCount++] = j;
			Player player = playerArray[j];
			player.anInt1537 = loopCycle; // make sure that the last time this player was updated was this cycle (to avoid removal during checking)
			int k = stream.readBits(1); // is there an update block?
			if(k == 1)
				anIntArray894[anInt893++] = j;
			int l = stream.readBits(1); // i think this is only used if the player is moving into our view? or the other way around possibly
			int i1 = stream.readBits(5); // the y position
			if(i1 > 15)
				i1 -= 32;
			int j1 = stream.readBits(5); // the x position
			if(j1 > 15)
				j1 -= 32;
	...
	{
		System.out.println("updatemask: " + i);
		
		if((i & 0x400) != 0) // force movement
		{
		...
		if((i & 0x10) != 0) // appearance
		{
			...
			aStreamArray895s[j] = stream_1; // cache the appearance
			player.updatePlayer(stream_1);
		}
		if((i & 2) != 0) // focus point
		{
			player.anInt1538 = stream.method436();
			player.anInt1539 = stream.method434();
		}
		if((i & 0x20) != 0) // first hit
		{
	...
	private void draw3dScreen()
	{
		...
		if(fpsOn)
		{
			...
			k += 15;
			aTextDrawingArea_1271.method380("Data: " + amountOfDataIn + "B/s, " + amountOfDataOut + "B/s", c, 0xffff00, k);
		}
		if(anInt1104 != 0)
		{
			int j = anInt1104 / 50;
...
*/
	...
	private long amountOfDataIn = 0;
	private long amountOfDataOut = 0;
	...
	private boolean parsePacket()
	{
		...
		try
		{
			...
			if(pktType == 123)
			{
				pktType = -1; // i hate this stupid packet
				return true;
			}
			if(pktType == 64)
			{
