...
public class Entity extends Animable {
	...
	public final void setPos(int i, int j, boolean flag)
	{
		if(anim != -1 && Animation.anims[anim].anInt364 == 1)
			anim = -1;
		if(!flag)
		{ // i don't understand what this block of code does. i think it might be used to make it seem like the player is running towards the point that they're appearing at.
			int k = i - smallX[0]; // previous position? 0?
			int l = j - smallY[0];
			if(k >= -8 && k <= 8 && l >= -8 && l <= 8)
			{
				if(smallXYIndex < 9)
