...
public final class Stream extends NodeSub {
	...
	public void doKeys()
	{
		int i = currentOffset; // take note of how long the encrypted data block is
		currentOffset = 0; // reset to 0
		byte abyte0[] = new byte[i]; // create an array to read encrypted data from
		readBytes(i, 0, abyte0); // read into abyte0
		BigInteger biginteger2 = new BigInteger(abyte0); // create a single integer from the block
		BigInteger biginteger3 = biginteger2.modPow(RSA_EXPONENT, RSA_MODULUS); // create a different integer based on the public key
		byte abyte1[] = biginteger3.toByteArray(); // create a byte array out of the encrypted message
		currentOffset = 0; // reset the offset back to 0 (if it isn't already)
		writeWordBigEndian(abyte1.length); // write the length of the encrypted block
		writeBytes(abyte1, abyte1.length, 0); // write the encrypted block
		//System.out.println("encrypted block length: " + abyte1.length);
	}
	...
	private static final BigInteger RSA_MODULUS = new BigInteger("");
